  ...
  UITextFieldDelegate, SFSafariViewControllerDelegate
{
  ...
  override func viewDidLoad() {
    ...
    self.valueStepper.minimumValue = -10_000_000
    self.valueStepper.maximumValue = 1_000_000
  ...
  override func viewDidAppear(_ animated: Bool) {
    ...
    Task { @MainActor in
      do { try await self.updateGoalAndInterface() } catch {
        logger.error("Error refreshing details for goal \(self.goal.slug): \(error)")
      }
    }
  }
  @objc func timerButtonPressed() { coordinator?.showTimerForGoal(goal) }
  ...
  func textFieldShouldReturn(_ textField: UITextField) -> Bool {
    if textField.isEqual(self.commentTextField) { self.submitDatapoint() }
    return true
  }
    ...
    -> Bool
  {
    if textField.isEqual(self.valueTextField) {
      ...
      if textField.text!.components(separatedBy: ".").count > 1 { if string == "." || string == "," { return false } }
      if string == "," {
        textField.text = textField.text! + "."
        return false
      }
      // Only allow a single : time separator
      if textField.text!.components(separatedBy: ":").count > 1 && string == ":" { return false }
      if (string as NSString).rangeOfCharacter(from: CharacterSet(charactersIn: "1234567890.:").inverted).location
        != NSNotFound
      {
        return false
      }
  ...
  @objc func submitDatapoint() {
    Task { @MainActor in
      ...
      do { try await self.goalManager.refreshGoals() } catch {
...
extension DateFormatter {
  ...
  fileprivate static func urtextDateString(from date: Date) -> String { urtextDateFormatter.string(from: date) }
}
...
extension DateFormatter {
  ...
  fileprivate static func dateTextFieldString(from date: Date, isDifferentYear: Bool, isDifferentMonth: Bool) -> String
  {
    if isDifferentYear {
