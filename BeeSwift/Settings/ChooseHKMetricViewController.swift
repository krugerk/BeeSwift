...
extension ChooseHKMetricViewController: UITableViewDelegate, UITableViewDataSource {
  ...
  var sortedMetricsByCategory: [HealthKitCategory: [HealthKitMetric]] {
    ...
    for category in HealthKitCategory.allCases { result[category] = sortedHKMetrics.filter { $0.category == category } }
    return result
  }
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    cell.metric = self.sortedMetricsByCategory[section]![indexPath.row]
